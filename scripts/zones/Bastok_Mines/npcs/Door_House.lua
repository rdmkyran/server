...
entity.onTrade = function(player, npc, trade)
    if npc:getID() == ID.npc.LELEROON_BLUE_DOOR then
        ...
        if
            letterBlue == 2 and
            npcUtil.tradeHas(trade, { xi.items.MYTHRIL_SHEET, xi.items.SQUARE_OF_KARAKUL_LEATHER, xi.items.SQUARE_OF_LM_BUFFALO_LEATHER, xi.items.SQUARE_OF_WOLF_FELT })
        then
            player:startEvent(521) -- accepts materials, now bring me 4 imperial mythril pieces

        elseif
            letterBlue == 3 and
            npcUtil.tradeHas(trade, { { xi.items.IMPERIAL_MYTHRIL_PIECE, 4 } })
        then
