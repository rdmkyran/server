...
entity.onTrade = function(player, npc, trade)
    ...
    if allNewC2000 == QUEST_ACCEPTED then
        if npcUtil.tradeHas(trade, { xi.items.INSECT_WING, xi.items.RABBIT_HIDE, xi.items.TWO_LEAF_MANDRAGORA_BUD }) then
    ...
    elseif legendaryPlanB == QUEST_ACCEPTED then
        if npcUtil.tradeHas(trade, { xi.items.LUMINICLOTH, xi.items.WOLF_HIDE, xi.items.REVIVAL_TREE_ROOT }) then
    ...
    elseif allNewC3000 == QUEST_ACCEPTED or allNewC3000 == QUEST_COMPLETED then
        if npcUtil.tradeHas(trade, { xi.items.BEETLE_SHELL, xi.items.HECTEYES_EYE }) then
...
entity.onEventFinish = function(player, csid, option)
    ...
    elseif
        csid == 314 and
        npcUtil.completeQuest(player, xi.quest.log_id.WINDURST, xi.quest.id.windurst.LEGENDARY_PLAN_B, { item = xi.items.SCENTLESS_ARMLETS, gil = 700 })
    then
        player:confirmTrade()
