...
entity.onTrade = function(player, npc, trade)
    ...
    if trade:hasItemQty(xi.items.POUCH_OF_PARRADAMO_STONES, 1) and trade:getItemCount() == 1 then
...
entity.onTrigger = function(player, npc)
    ...
    if player:hasKeyItem(xi.ki.MIASMA_FILTER) then
        ...
        if miasmaFilterCD >= os.time() then
            ...
            if player:hasItem(xi.items.POUCH_OF_PARRADAMO_STONES) or player:hasItem(xi.items.FLAXEN_POUCH) then
